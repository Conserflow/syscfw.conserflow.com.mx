...
class PartidaEntradaController extends Controller
{
  ...
  public function store(Request $request)
  {
    try
    {
      ...
      if (isset($partida_entrada_busqueda) == true)
      {
        ...
        {
          $this->actualiza_cantidad($request->req_com_id, $part_entr->cantidad, $request->cantidad,$request->cantidad_rechazo);
        }
        else
        {
          $this->guardar_cantidad($request->req_com_id, $request->cantidad, $request->cantidad_rechazo);
        }
      ...
      else
      {
        ...
        $partidaentrada->cantidad_rechazo = $request->cantidad_rechazo;
  ...
  public function guardar_cantidad($orden_compra_id, $cantidad, $cantidad_rechazo)
  {
    ...
    $requisis->cantidad_rechazo = floatval($cantidad_rechazo);
    $requisis->save();

    return true;
  }

  public function actualiza_cantidad($orden_compra_id, $cantidad, $cantidad_request, $cantidad_rechazo)
  {
    ...
    $requisis->cantidad_rechazo = (floatval($requisis->cantidada_rechazo) + floatval($cantidad_rechazo));
    Utilidades::auditar($requisis, $requisis->id);
    $requisis->save();
